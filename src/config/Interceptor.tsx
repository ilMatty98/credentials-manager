...
export const Interceptor: React.FC<InterceptorProps> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const requestInterceptor = instanceAxios.interceptors.request.use(
            ...
            (error: AxiosError) => {
                nascondiSpinner();
                mostraAlert(ALERT_COLOR.ERROR, <ExclamationTriangleIcon className="mt-px h-6 w-6" />, ALERTS.errorTitle, ALERTS.errorMessage);
        ...
        const responseInterceptor = instanceAxios.interceptors.response.use(
            (response: AxiosResponse) => {
                nascondiSpinner();
                mostraAlert(ALERT_COLOR.SUCCESS, <CheckCircleIcon className="mt-px h-6 w-6" />, ALERTS.successTitle, ALERTS.successMessage);
                return response;
            },
            (error: AxiosError) => {
                nascondiSpinner();
                if (error.response) {
                    ...
                    mostraAlert(ALERT_COLOR.ERROR, <ExclamationTriangleIcon className="mt-px h-6 w-6" />, ALERTS.errorTitle, message);
